...
#text(style: "italic", [
    In questo capitolo valuto il raggiungimento degli obiettivi, indico le direzioni in cui il lavoro può proseguire e riassumo ciò che ho imparato.
])
#v(1em)
...
#figure(
  ...
  table(
    ...
    [#obj("O01")], [Ricezione di eventi asincroni dalle altre componenti della piattaforma, con caricamento delle configurazioni del cliente e delle entità interessate], [Raggiunto],
    [#obj("O02")], [Valutazione di KPI specifici per entità e cliente, e interfacciamento con l'IA generativa per la produzione della sintesi testuale], [Raggiunto],
    [#obj("O03")], [Indicizzazione delle entità per il recupero di documenti non strutturati], [Non raggiunto],
    [#obj("O04")], [Ricezione di richieste utente in modalità agente, con recupero documentale e risposta], [Non raggiunto],
    [#obj("D01")], [Integrazione nell'architettura e nel ciclo di rilascio della piattaforma], [Non raggiunto],
  )
)<tab:obiettivi>

Gli obiettivi raggiunti sono quelli che riguardano i dati strutturati, e su di essi il lavoro è arrivato più lontano di quanto il piano richiedesse: non solo il servizio produce i riepiloghi, ma li conserva, li mantiene allineati ai dati e li rende consultabili. Al servizio si affianca il modello dati del semantic layer, dove le metriche sono state definite e da cui i valori provengono, più una suite di test automatici e un ambiente di sviluppo che riproduce l'infrastruttura con un comando.

Gli obiettivi #obj("O03") e #obj("O04") riguardano invece i dati non strutturati e non sono stati realizzati. La decisione, presa con il tutor aziendale a metà stage, è discussa nel @cap:introduzione[Capitolo] e le sue ragioni tecniche nel @cap:progettazione[Capitolo]: l'estensione ai documenti richiedeva un'infrastruttura autonoma di indicizzazione e recupero, il cui sviluppo avrebbe occupato tutto il tempo restante lasciando incompiuto anche il flusso sui dati strutturati. Fra due fronti aperti a metà e uno chiuso, si è preferito il secondo.

L'obiettivo desiderabile #obj("D01") non è stato raggiunto per ragioni di tempo e di coordinamento con il ciclo di rilascio interno, e nella settimana conclusiva l'azienda ha preferito destinare il tempo residuo a un affiancamento formativo con uno sviluppatore del team.
...
Ho imparato a lavorare con i modelli linguistici, e soprattutto a diffidarne: la loro affidabilità dipende dal compito assegnato, perché lo stesso modello che inventa un numero se gli si chiede di calcolarlo, lo espone fedelmente se glielo si fornisce. Buona parte della progettazione descritta in questa relazione consiste nel disegnare il sistema attorno a questa distinzione.

Ho conosciuto il semantic layer, che non avevo mai incontrato negli studi, e ne ho scritto i modelli. Definire una metrica si è rivelato meno meccanico di quanto pensassi: il tempo non se ne va nello scriverla, ma nel decidere quali dati prendere e che cosa debbano rappresentare.

Ho lavorato con la comunicazione asincrona a code, e ho capito l'importanza dell'idempotenza.

Sul piano non tecnico, l'acquisizione più utile riguarda i requisiti. Solitamente in ambito accademico arrivano già scritti; in azienda vanno ricavati, perché stanno nella testa delle persone e vanno distinti dalle soluzioni che ha già in mente chi li esprime. Alcuni però non stanno nella testa di nessuno, e compaiono soltanto quando qualcuno guarda un vero risultato.
